...
{   
    ...
    else printf("INFO: Server Socket successfully created..\n");
    ...
    while(1)
    {
        ...
        for(int i=0;i<MAX_CONNECTIONS;i++)
        {
            ...
            if (conns[i].client>-1 && FD_ISSET(conns[i].client, &read_set))
            {
                ...
                int size=0, k=0;
                while(1){
                    //clear temporary receiver buffer
                    bzero(&temp_recv, sizeof(temp_recv));
                    temp_recv[0] = '\0';
                    // receive data from client
                    
                    n = recv(conns[i].client, temp_recv, TEMP_BUF_SIZE, 0);
                    if(n>0)
                    {
                        temp_recv[n]='\0';
                    ...
                    {
                        //k indicates the end of the HTTP request header "\r\n\r\n"
                        while(k-4<=strlen(buffer_recv))
                        {
                            if(buffer_recv[k]=='\r' && buffer_recv[k]=='\n' && buffer_recv[k]=='\r' && buffer_recv[k]=='\n') break;
                        }
                        break;
                    }                    
                }
